...
from datautils import score, make_batches
...
#        return load_glove_embedding(path, dim, self.tokenizer.word_index)
    ...
    def _build_model(self, n_classes=2, embedding_dim=256, embeddings_weights=None):
...
#            embedding_weights = self._generate_embedding(embeddings_path, embedding_dim)
        ...
        word_encoder = Bidirectional(
            GRU(128, return_sequences=True, kernel_regularizer=l2_reg))(embedded_word_seq)
        ...
        sentence_encoder = Bidirectional(
            GRU(128, return_sequences=True, kernel_regularizer=l2_reg))(attention_weighted_sentences)
...
#        self.reverse_word_index = {value:key for key,value in self.tokenizer.word_index.items()}
    ...
    def train(self, train_text, train_label, val_X, val_y,
        batch_size=64, epochs=1, 
        embedding_dim=256,
        embeddings=np_embedding_weights, 
        saved_model_dir='./vis_utils/saved_models', saved_model_filename=None,):
        # fit tokenizer
        ...
        self.model = self._build_model(
            ...
            embeddings_weights=np_embedding_weights)
        ...
        train_batch_generator = han_batch_generator
        for i in range(6):
            if i == 2:
            ...
            if i == 4:
                ...
                K.set_value(self.model.optimizer.lr, 0.00002)
            self.model.fit_generator(train_batch_generator(train_text, train_label, batch_size=batch_size), 
                       epochs=1, 
                       callbacks=callbacks,
...
#        pdb.set_trace()
        ...
        reconstructed_texts = [[idx_dict[int(i)] 
